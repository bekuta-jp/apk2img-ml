...
def train_doc2vec(
    ...
    # ★ADDED: 学習進捗を tqdm で表示するため、エポックごとに train() を呼び出す
    total_examples = len(documents)              # コーパス中のドキュメント数
    start_alpha = model.alpha                    # 初期学習率を退避
    end_alpha = model.min_alpha                  # 最終学習率（デフォルト値）
    if epochs > 0:
        alpha_delta = (start_alpha - end_alpha) / epochs
    else:
        alpha_delta = 0.0

    # ★CHANGED: まとめて train していた部分を、エポックループ＋tqdm に変更
    for epoch in tqdm(range(epochs), desc="Training Doc2Vec"):
        ...
        # 学習率を徐々に下げる（gensim が推奨するパターンに合わせる）
