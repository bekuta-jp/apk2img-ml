# docs index

`apk2img-ml` の運用コードは `src/apk2img_ml/` にあります。

## 最初に読む

- [../README.md](../README.md): 全体概要と実行例
- [setup.md](setup.md): 依存関係と環境構築（pip / 外部ツール / conda）
- [programs.md](programs.md): 新CLIと各モジュールの仕様
- [manuals/tune-cnn.md](manuals/tune-cnn.md): `tune-cnn` の個別マニュアル
- [current_changes.md](current_changes.md): 再構成と `train_eval_mrun` 移植の現状

## 重要

- `src/apk2img-ml/legacy/` は旧実験コードの保管領域です。
- 今後の開発対象は `src/apk2img_ml/` です。
